...
#   --worst-by suction            rank those episodes by one class's error COUNT
#   --worst-by suction:rate       ...by that class's error RATE instead, so a long
#                                 recording cannot crowd out a short one that gets
#                                 the class badly wrong (`rate` alone = all classes)
